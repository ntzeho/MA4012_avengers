const short DEFAULT_MOTOR_DRIVING_SPEED = 40;
const short SLOW_MOTOR_DRIVING_SPEED = 20;
const short DEFAULT_MOTOR_TURNING_SPEED = 35;
const short DEFAULT_BALL_MOTOR_SPEED = 127;
const short MS_PER_DEGREE = 8; //line sensors turning
const short DEFAULT_MOTOR_TURNING_NORTH_SPEED = 80;
const short DEFAULT_MOTOR_HOME_DRIVING_SPEED = 60;
const short DEFAULT_MOTOR_START_DRIVING_SPEED = 127;

const short START_MOVE_FIELD_DISTANCE = 150;
const short END_MOVE_FIELD_DISTANCE = 120;

const short START_MOVE_FIELD_TIME = 1800;
const short END_MOVE_FIELD_TIME = 2000;
const short TIME_TO_TRAVEL_10_CM = 350;
const short TIME_TO_TRAVEL_15_CM = 500;
const short TIME_TO_TRAVEL_30_CM = 900;
const short TIME_TO_TRAVEL_60_CM = 1600;
const short DEFAULT_LINE_SENSOR_SLEEP = 100;

const short ROTATE_20_DEGREES_SLEEP = 200;
const short ROTATE_45_DEGREES_SLEEP = 450;

const short BALL_OUT_MOTOR_SLEEP_TIME = 3000;
const short ROBOT_REAR_DISTANCE_THRESHOLD = 10;
const short ROBOT_FRONT_DISTANCE_THRESHOLD = 10;
const short ROBOT_BALL_DISTANCE_THRESHOLD = 50;
const short ROBOT_BALL_SHORT_DISTANCE_THRESHOLD = 40;
const short ROBOT_BALL_REJECTION_DISTANCE_THRESHOLD = 40;
const short ROBOT_BALL_REJECTION_SHORT_DISTANCE_THRESHOLD = 30;
const short ROBOT_SENSOR_DIFF_THRESHOLD = 5;


const short ROBOT_STATE_CORRECT_VALUE = 1;
const short FRONT_DIST_CORRECT_VALUE = 1;

const short MAX_BOUNDARY_BALL_TRIES = 5;